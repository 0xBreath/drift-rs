...
impl ToAccountInfo for AcctCtx<Account> {
    fn to_account_info<'b>(
        &self,
        _: Pubkey,
        signs: bool,
        ...
        self.account
            .to_account_info(self.key, signs, writable, exec)
    }
}
...
impl ToAccount for SubscribeUpdateAccountInfo {
    fn to_account(&self) -> anyhow::Result<Account> {
        Ok(Account {
            ...
            owner: Pubkey::try_from(self.owner.clone())
                .map_err(|_| anyhow::anyhow!("Failed to convert owner to pubkey"))?,
            executable: self.executable,
            rent_epoch: self.rent_epoch,
        })
    }
